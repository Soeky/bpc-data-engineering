...
class Evaluator:
    ...
    def __init__(
        ...
        self.matcher = RelationMatcher(match_type=match_type)
    ...
    def evaluate(
        ...
        for pred, gold in zip(predictions, gold_relations_list):
            ...
            self.logger.info(
                ...
                f"(gold relations file: {gold_file_name})"
            )
            ...
            # Match relations
            true_positives, false_positives, false_negatives, partial_matches = self.matcher.match(
                predicted_relations,
                gold_relations
            )
            ...
            self.logger.info(
                ...
                f"Partial Matches (entities correct, type wrong)={partial_count}"
            )
            
            # Log partial matches (entities match but type differs)
            if partial_matches:
                self.logger.debug(f"\n[Evaluator] Document {doc_id} - Partial Matches (entities correct, type wrong):")
                for i, (pred, gold) in enumerate(partial_matches, 1):
            ...
            # Calculate fuzzy metrics (treating partial matches as correct for entities)
            fuzzy_tp = tp_count + partial_count
            fuzzy_fp = fp_count - partial_count  # Exclude partial matches from FP
            fuzzy_fn = fn_count
            ...
            result = EvaluationResult(
                doc_id=doc_id,
                true_positives=true_positives,
                false_positives=[],  # FPs are ParsedRelations, not Relations
                false_negatives=false_negatives,
                partial_matches=partial_matches,  # Store partial matches
                ...
                bertscore=0.0,  # TODO: Implement BERTScore
